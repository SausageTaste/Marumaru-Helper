...
import marumaru_helper_data as mdata
...
DATA_FOL_LOC_S = "marumaru_helper\\"
...
def first_set_up():
    print("func called: first_set_up")

    os.mkdir(DATA_FOL_LOC_S)

    with open(SAVE_TXT_S, 'w', encoding="utf8") as file:
        file.write(mdata.SAVE_DATA_S)

    os.mkdir(THUMB_FOL_S)

    img_dir_s = THUMB_FOL_S + "6855.jpg"
    g = open(img_dir_s, "bw")
    g.write(base64.decodebytes(mdata.THUM_64_S))
    g.close()
...
def prints(*args):
    ...
    try:
        ...
        for x in (DEFAULT_ENCODING_S, "cp437"):
            try:
                print(text_s.encode(encoding="utf8", errors="ignore"))#.decode(encoding=x, errors="ignore"))
...
class UiMainWindow(QtGui.QMainWindow):
    ...
    def retranslateUi(self):
        ...
        self.btn_option.setToolTip(self.data.get_value("ui", "main-btn_option-tooltip"))
    ...
    def on_first_start(self):
        data = __import__("marumaru_helper_data", fromlist=["marumaru_helper_data"])
        NoticeWindow(data.WELCOME_S, self).exec_()
        del data
    ...
    def btn_add_f(self):
        ...
        #### commands ####
            ...
            if args_len_i == 2:
                ...
                return
        elif args_l[0] == "con_prints":
            if args_len_i == 2:
                prints(args_l[1])
                self.label_1.setText('Command "con_prints" : Called function print({}).'.format(args_l[1]))
                self.lineEdit.clear()
                return
            else:
...
class Item:  # QtGui.QMainWindow
    ...
    def retranslate(self):
        ...
        self.btn_load.setToolTip(self.ui_class.data.ld_ui_get_a_value("item-btn_load-tooltip"))
    ...
    def show_thum(self, option=2):
        if not os.path.isfile(self.get_img_dir_s()):
...
class OptionWindow(QtGui.QDialog):
    ...
    def everytime(self):
        ...
        if option_i:
            ...
            self.com_main_startup_sort_reverse.setDisabled(False)
        else:
            self.btn_general_sort_now.setDisabled(True)
            self.com_main_startup_sort_reverse.setDisabled(True)
